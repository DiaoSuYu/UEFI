...
* The first 4 defines are key words for the DEC spec.
* The DEC Spec number is what version of the EDK II DEC spec this package is at
* All things EDK II get a GUID see https://www.guidgenerator.com/ to get a new GUID
* The Package Version is User defined and is Number dot Number 
...
This is to use relative paths throughout the EDK II source code  
```
[Includes]
  Include
```
...
* These are the libraries created by this package. Notice each is pointing to a .h file.
* The Library Base Name is on the left the a vertical bar followed by the relative path to the interface file.
* Each of these Libraries are produced by this package
...
* If this package is producing a protocol (function interface) then the function will be listed here.
* This allows other Modules to discover these protocols and use them.
...
* If there are data structures produced by this package, they will need a GUID to identify them.
* *Remember*, UEFI / EDK II only know about GUIDs and not a user friendly name.
* The fist example is the GUID for the PCDs
* All Packages will probably have some PCDs that it will define and thus will define a GUID to associate with the PCD data structures.
...
* The PCDs in this section are all produced / created by this package.
* They are unique because of the GUID from the GUID section.  
* The PCDs listed will be given the best known default to start with and then if a Platform would like a different value it should be updated in the corresponding .DSC for the platform.

**NOTE** It is important to have the last number for each defined PCD be a Token value defined as a unique hex number for this file.
...
If it is a patchable PCDs, other tools can change the values outside of the Build
